	...
	<view>
		...
		<view class="searchTopBox">
			...
			<view class="searchBoxRadius">
				<input class="inputIocale" type="search" v-model="searchValue" @confirm="searchNow" placeholder="搜索酒店名称" />
			...
			<view class="Tk_scrollview" @click="godetail(item)">
				<view class="Tk_item">
					...
					<view class="Tk_bacg">
						...
						<text class="Tk_text2">{{item.address}}&nbsp;|&nbsp;{{item.tel}}</text>
		...
		<view :hidden="screenIndex == 3">
			<view class="Tk_scrollview" v-for="(item,index) in scenicList" :key="index" v-if="index < scenicListIndex " @click="godetail(item)">
				<view class="Tk_item">
					...
					<view class="Tk_bacg">
						<text class="Tk_text1">{{item.name}}</text>
						<text class="Tk_text2">地址：{{item.address}}</text>
						<text class="Tk_text2">电话：{{item.tel}}</text>
						<!-- <text class="Tk_text3" v-if="item.ticketAdultPrice!=='暂无门票价格信息！'">¥{{priceConversion(item.ticketAdultPrice)}}元起</text> -->
					</view>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			lyfwData:function() {
				...
				// 请求景区列表
				uni.request({
					url:'http://218.67.107.93:9210/api/app/hotelList',
					method:'GET',
					success:(res) => {
						console.log(res)
						if (res.data.msg == '获取酒店列表信息成功！') {
							this.scenicList = res.data.data;
							uni.hideLoading()
							uni.stopPullDownRefresh();
						} else if (res.data.msg == '查不到相关酒店，请确认酒店名！') {
							this.scenicList = '';
							uni.hideLoading()
							uni.stopPullDownRefresh();
							uni.showToast({
			...
			godetail: function(e) {
				...
				if(e.ticketTitle == '武夷山风景名胜区'){
					...
					uni.navigateTo({
						url: '/pages/JDDG/scenicSpotTickets/ticketsDetails?hoteld='+e.id,
					})
				}
