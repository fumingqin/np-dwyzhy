...
<script>
	...
	export default {
		...
		onLoad(options) {
			var that=this;
			that.hoteld = options.hoteld;
			console.log(that.hoteld,'11111111111111');
			that.lyfwData(that.hoteld);
		},
		...
		methods: {
			...
			lyfwData: function(e) {
				...
				uni.request({
					url: 'http://218.67.107.93:9210/api/app/getHotelDetail',
					method:'GET',
					data: {
						hotelId: that.hoteld,
					},
					success: (res) => {
						...
						uni.stopPullDownRefresh()
						that.scSpotContent = res.data.data;
					}
				})
